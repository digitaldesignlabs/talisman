...
/**
* Interface function for setting a flag that will display certain debug information
*
...
* @param {boolean||string||number} bool. Sets the variable to true or false.
*/
let escapeTagGlobal = false;
function setEscape(bool) {
    if (bool === "true" || bool === 1) {
        escapeTagGlobal = true;
    } else if (bool === "false" || bool === 0) {
        escapeTagGlobal = false;
    } else {
        escapeTagGlobal = !!bool;
    }
}
...
/**
* A function that takes a nested template array and flattens it while also inserting blocks that were loaded from external sources.
*
* @param {array} array. Nested template array generated by parseForTags()
* @param {object} blocks. Object containing block objects, generated by templateLoad()
...
/**
* A function that takes aa template object and parses it into a stream ready to be rendered or sent to another template.
*
* @param {object} processObject. An object containing various pieces of information, packaged into an object to make
*        recursion easier. This object is initially created using input from the user via the templateLoad() function.
...
function contentStream(processObject, index, done) {
    ...
    const streamElement = function (element) {
        ...
        if (element.content) {
            // The readFile function returns a promise, so any block inserted from an external file will be returned as a promise.
            if (element.content.constructor === Promise) {
                element.content.then(function (content) {
                    element.content = content;
                    streamBlock(element);
                }).catch(function () {
                    // If this promise fails to resolve, then most likely the template file for this block could not be found
                    // So push an error and move on.
    ...
    if (typeof element === 'object') {
        ...
        // A flag can be set on a per-block basis as to whether or not to render tags that have no variable associated with them
...
function getTemplate(file) {
    ...
    return (function () {
        ...
        if (cache[templatePath] === undefined) {
            cache[templatePath] = readFile(templatePath, "utf8")
                .then(function (templateString) {
                    // strip comments out of the template.
...
function templateLoad(templateFile) {
    ...
    // Context object for exposing interface methods to the user seperately, so that they can't mess with template values directly
    // This object is returned by every method so that methods can be chained such as example.load(args).set(args).render();
    const ctx = {};
    ...
    const set = function (inputObject, blockName) {
        if (blockName === undefined) {
            if (isKeyValuePair(inputObject)) {
                ...
                throw new Error("If there is no block name specified, you must supply an inputObject with key:value pairs");
            }
        } else {

            if (template.vars[blockName]) {
                // If there's already an inputObject there, merge them.
                Object.assign(template.vars[blockName], inputObject);
            } else {
                // If its undefined, an array or a string or something, just overwrite anything that is already there.
    ...
    const setEscapeTags = function (blockName) {
        template.escapeTags[blockName] = true;
        return ctx;
    };
    ...
    outputStream._read = function () {
        // It is necessary to define a _read function for the stream, however in our implementation we are pushing items onto
        // the stream directly so this function is not needed. If this function is not defined then you will be spammed with
        // 'not implemented' errors.
        return undefined;
    };

    const render = function (blockNames) {
        // The templateFile should be a promise that reads a file, and then processes it into an array ready to be streamed.
        template.file
            .then(function (templateArray) {
                // If there is a blockNames argument in the render function, then extract the specified block for rendering
                ...
                // contentstream is a recursive function that takes each element of the array, processes it if it is a tag, and then streams it.
                contentStream({
                    ...
                    output: outputStream
                    // The index that the function should start at, and the action to be completed when this function finishes
                }, 0, function () {
...
/**
* Function for exposing the implementation methods of the library for the purposes of unit testing.
*
* @return {object} - An object containing all functions, methods and some variables
*/
