 ...
 *
 * @access private
 * @param {Promise?<string>} templateString - A string describing this template, or a Promise for the same
...
function createTemplate(templateString) {
    return Promise.resolve(templateString).then(content => {
        ...
        const block = new Block(content, root, null, null, blockmap);
 ...
 * Input is memoized, so we only hit the disk on the first go.
 *
 * @access public
 * @param {string} filename
 * @return {Promise<object>} An object exposing the Talisman API
 */
const createTemplateFromFile = (function () {
    const memo = new Map();
    return function (fileName) {

        if (memo.has(fileName) === true) {
            return createTemplate(memo.get(fileName));
        }

        return readFile(fileName, "utf8").then(content => {
            memo.set(fileName, content);
