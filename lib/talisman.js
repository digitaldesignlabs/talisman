...
* @param {boolean||string||number} bool. Sets the variable to true or false.
*/
let stripTagsDefault = false;
function setStripTagsDefault(bool) {
    stripTagsDefault = !!bool;
}
...
function templateLoad(templateFile) {
    ...
    // This function sets
    const setStripTagsWithinBlock = function (blockName, bool) {
        if(bool === undefined) {
            bool = true;
        }
        template.stripTags[blockName] = !!bool;
        return ctx;
    };
    ...
    ctx.error = error;
    ctx.set = set;
    ctx.stripTagsInBlock = setStripTagsWithinBlock;
    ctx.remove = remove;
    ctx.restore = restore;
    ctx.addMask = addMask;
    ctx.render = render;
