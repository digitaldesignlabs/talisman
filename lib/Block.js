...
class Block {
     ...
     * @constructor
     */
    constructor(content, name, vars, masks, blocks) {
        ...
        // Keep a copy of the blockmap, so we can lookup blocks by name. Only the root block really does this,
        // but the child blocks all add themselves to the blockmap
        this.blocks = blocks || new Map();
        this.blocks.set(this.name, this);
    ...
    seekByName(blockName) {
        ...
        return this.blocks.get(fullName);
    }
     ...
     * @todo add support for maps, promises
     *
     ...
     * @todo add support for object streams
     *
     * @param {Array|Promise} raw - An array, or Promise for the same
     * @return {undefined}
     */
    setDataSource(raw) {
    ...
    addChild(name, content) {
        this.content.then(() => {
            ...
            this.vars[name] = new Block(content, fullName, this.vars, this.masks, this.blocks);
        });
    }
     ...
     * ready()
     *
     * Returns a promise which resolves when this block is ready for rendering
     *
     * @return {Promise<string>}
     */
    ready() {
        return Promise.all(this.blockIsReady)
            .then(() => this.content);
    }
