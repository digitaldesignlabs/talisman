...
class Block {
     ...
     * @constructor
     */
    constructor(content, name, vars, masks, blockmap) {
        ...
        this.content = Promise.resolve(content).then(content => {
            ...
            return content.replace(blockRegex, (original, blockName, blockContent) => {
                const child = this.addChild(blockName, blockContent, blockmap);
                this.processing.push(child);
                return `{{${blockName}}}`;
            });
        });
    ...
    addChild(name, content, blockmap) {
        return this.content.then(() => {
            ...
            this.vars[name] = new Block(content, fullName, this.vars, this.masks, blockmap);
            return this.vars[name].processed();
        });
    }
