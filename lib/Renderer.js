...
class Renderer {
    ...
    constructor(block) {
        ...
        if (block.dataSource === undefined) {
            ...
            this.queue = this.block.ready().then(templateText => this.parseForTags(templateText));
            ...
            this.queue = block.dataSource.then(data => {
                ...
                function createRowBlock(vars) {
                    ...
                    vars.talismanRowIsEven = counter % 2 === 0;
                ...
                // Fairly rudimentary object stream support. Would be better if we could make this more
                // streamy though -- and render the blocks as the data source emits, rather than doing them
                // all up-front like we are now?
                ...
                if (data instanceof ReadableStream) {
                    ...
                    return new Promise((resolve, reject) => {
                        data.on("end", () => resolve(rows))
                            .on("error", err => reject(err))
                            .on("data", vars => {
                                rows.push(createRowBlock(vars));
                            });
                    });
                }

                return data.map(vars => createRowBlock(vars));
            });
        }
    }
    ...
    processElement(element) {
        return Promise.resolve().then(() => {
            ...
            // Strings we can return right away
            if (typeof element === "string") {
                return element;
            }

            // If there is nothing there, just return empty string
            if (utils.isNullOrUndefined(element) === true) {
                return "";
            }
            ...
            // Tags, we load their data and recurse
            ...
            // Promises we resolve and continue
            if (element instanceof Promise) {
                return element.then(data => this.processElement(data));
            }
    ...
    processQueue(queue) {
        ...
        this.processElement(element).then(data => {
            ...
            // Data should now be one of a string, buffer, or stream.

            // If we only have an empty value, then just move on
            if (utils.isNullOrUndefined(data) || data === "") {
                return this.processQueue(queue);
            }

            // If it is a stream, pipe it to our output stream
            if (data instanceof ReadableStream) {
                data.on("end", () => this.processQueue(queue)).pipe(this.output, {end: false});
                return;
            }

            // If the data should be escaped, escape it
            if (element.escape === true && this.isStringOrBuffer(data)) {
                data = htmlescape(data);
            }

            // Push the data through any masks which are interested.
            // Masks come after escaping, so masks can include markup if they choose.
            if (element.masks) {
            ...
            // If this element is a string, push it down the stream. Otherwise, toString() it and then push it.
            if (this.isStringOrBuffer(data)) {
    ...
    isStringOrBuffer(maybe) {
        return maybe && (typeof maybe === "string" || Buffer.isBuffer(maybe));
    }
